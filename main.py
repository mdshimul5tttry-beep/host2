...
class Btn(types.InlineKeyboardButton):
    """InlineKeyboardButton with Telegram Bot API 9.4 style colors.
    style="primary"=Blue | "success"=Green | "danger"=Red
    Graceful fallback: older clients ignore unknown fields.
    """
...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8289163371:AAGhQB7brOJ0x4UDIQBlX1TKnolM0ixrxRw"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "7909873127"))
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    # Payment methods disabled — plans unlock via redeem codes only
}
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wᴇʟᴄᴏᴍᴇ",         "#0F172A", "Pᴀʏ Hᴏꜱᴛɪɴɢ"),
...
def payments_kb(plan: Optional[str] = None) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    # Payment methods disabled — redirect to coupon redeem
    kb.add(Btn(f"{G['ticket']}  {sc('Enter Coupon Code')}", callback_data="menu_coupon", style="success"))
...
def render_payment_methods_for(call: types.CallbackQuery, plan: str) -> None:
    ...
    price = p.get("price", 0)
    if price == 0:
        # Free plan — no code needed
        cap = (
            f"<b>{G['ok']} {sc('Free Plan')}</b>\n"
            f"{G['div_eq']}\n"
            f"{bullet('Plan', p['name'])}\n"
            f"{sc('This plan is free. No code required')}.{FOOTER}"
        )
        kb = types.InlineKeyboardMarkup()
        ...
        return
    cap = (
        f"<b>{G['key']} {sc('Redeem Code to Unlock')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Plan',  p['name'])}\n"
        f"{bullet('Price', '{}$'.format(price))}\n"
        f"{G['div']}\n"
        f"<b>{sc('How to get this plan')}:</b>\n"
        f"1. {sc('Get a redeem code from admin')}\n"
        f"2. {sc('Open Coupon menu and enter the code')}\n"
        f"3. {sc('Then come back and select this plan')}\n"
        f"{G['div']}\n"
        f"📩 {sc('Get code from admin')}: @bd_top_admin\n"
        f"{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(f"{G['ticket']}  {sc('Enter Coupon Code')}", callback_data="menu_coupon", style="success"))
...
def approval_required() -> bool:
    # Approval system disabled — uploads go live immediately
    return False
...
def replace_menu_video(key: str, file_bytes: bytes, ext: str = ".mp4") -> bool:
    """Save admin-uploaded video as menu banner for `key` (or 'default').
    Users then see this video on every matching menu instead of photo.
    """
    key = (key or "default").lower().strip()
    if key != "default" and key not in _PHOTO_SPECS:
        return False
    out_dir = DIRS["videos"]
    out_dir.mkdir(parents=True, exist_ok=True)
    ext = ext if ext.startswith(".") else f".{ext}"
    if ext.lower() not in (".mp4", ".mov", ".webm", ".mkv"):
        ext = ".mp4"
    out = out_dir / f"{key}{ext}"
    try:
        # remove old variants for same key
        ...
        if key == "default":
            for k in list(_PHOTO_SPECS.keys()) + ["main", "welcome"]:
                VIDEOS.setdefault(k, str(out))
        _VIDEO_FILE_IDS.clear()
        try:
            if gh_enabled():
                threading.Thread(
                    target=lambda: _gh_put_file(
                        f"storage/videos/{key}{ext}",
                        file_bytes,
                        f"chore(videos): admin updated banner video '{key}'",
                    ),
                    daemon=True,
                ).start()
        except Exception as _e:
            print(f"[replace_menu_video] gh mirror skipped: {_e}")
        return True
    except Exception as e:
        print(f"[replace_menu_video] {key}: {e}")
        return False
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    # ── Approval gate DISABLED — always go live ─────────────────
    doc_db["approval_status"] = "approved"
    doc_db["status"] = "stopped"
    save_bot(doc_db)
    db = db_load()
...
def _handle_coupon_user(m: types.Message) -> None:
    ...
    plan = (c.get("plan") or "").strip().lower()
    # If coupon has a plan field, grant that plan directly (100% style unlock)
    if plan and plan in PLAN_LIMITS:
        c["uses_left"] = int(c["uses_left"]) - 1
        db_save(d)
        ok = grant_plan(m.from_user.id, plan)
        if ok:
            bot.reply_to(
                m,
                f"<b>{G['ok']} {sc('Plan unlocked')}</b>\n"
                f"{bullet('Code', code)}\n"
                f"{bullet('Plan', PLAN_LIMITS[plan]['name'])}\n"
                f"{sc('Enjoy your premium access')}!",
                parse_mode="HTML",
            )
        else:
            bot.reply_to(m, f"{G['no']} {sc('failed to grant plan')}")
        return
    # 100% coupon without plan → unlock highest paid tier (lifetime) or pro
    if pct >= 100:
        target = "lifetime" if "lifetime" in PLAN_LIMITS else "pro"
        c["uses_left"] = int(c["uses_left"]) - 1
        db_save(d)
        ok = grant_plan(m.from_user.id, target)
        bot.reply_to(
            m,
            f"<b>{G['ok']} {sc('Premium unlocked')}</b>\n"
            f"{bullet('Code', code)}\n"
            f"{bullet('Plan', PLAN_LIMITS.get(target, {}).get('name', target))}\n"
            f"{sc('Code redeemed successfully')}!",
            parse_mode="HTML",
        )
        return
    # Partial % → wallet credit (old behaviour)
...
def _handle_coupon_admin(m: types.Message) -> None:
    ...
    if op == "add" and len(parts) >= 4:
        ...
        plan = parts[4].lower() if len(parts) >= 5 else ""
        if plan and plan not in PLAN_LIMITS:
            bot.reply_to(m, f"{G['no']} {sc('unknown plan')}. Use: free/starter/basic/pro/enterprise/lifetime"); return
        entry = {"percent": pct, "uses_left": uses}
        if plan:
            entry["plan"] = plan
        d["coupons"][code] = entry
        db_save(d)
        audit(m.from_user.id, "coupon_add", f"code={code} pct={pct} uses={uses} plan={plan or '-'}")
        msg = f"{G['ok']} {sc('added')} {code}"
        if plan:
            msg += f" → {plan}"
...
def _progress_bar_broadcast(current, total, width=12):
    return _progress_bar(current, total, width=width)
...
def action_adm_export_full_db(call):
    ...
    fname = f"sir_linuxx_db_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
    import io
...
def action_adm_export_users_csv(call):
    ...
    fname = f"sir_linuxx_users_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_bots_csv(call):
    ...
    fname = f"sir_linuxx_bots_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_trans_csv(call):
    ...
    fname = f"sir_linuxx_transactions_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_audit_csv(call):
    ...
    fname = f"sir_linuxx_audit_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
    ...
    try:
        ...
        with target.open("rb") as fh:
            bot.send_document(
                ...
                parse_mode="HTML",
                visible_file_name=f"sir_linuxx_backup_{stamp}.zip",
            )
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        link = f"https://t.me/PayHosting_bot?start={uid}"
    cap = (
        f"<b>{G['users']} {sc('Referral')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Your link', link)}\n"
...
def render_adm_photos(call: types.CallbackQuery) -> None:
    if not is_owner(call.from_user.id) and not admin_can(call.from_user.id, "manage_admins"):
        ack(call, "Owner / full-access only."); return
    has_default_vid = bool(VIDEOS.get("default"))
    n_vids = len([k for k in VIDEOS if k != "default"])
    cap = (
        f"<b>{G['upload']} {sc('Menu Photos & Videos')}</b>\n"
        f"{G['div_eq']}\n"
        f"{sc('Tap a menu, then send PHOTO or VIDEO as banner')}.\n"
        f"{bullet('Default video', 'SET' if has_default_vid else 'not set')}\n"
        f"{bullet('Custom videos', n_vids)}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    for key, label in sorted(PHOTO_KEYS_FRIENDLY.items()):
        if key in _PHOTO_SPECS:
            mark = "🎬" if key in VIDEOS else "🖼️"
            kb.add(Btn(f"{mark}  {sc(label)}", callback_data=f"adm_photo_{key}", style="primary"))
    kb.add(
        Btn("🎬  Default Video (all menus)", callback_data="adm_video_default", style="success"),
        Btn("🗑️  Clear All Videos", callback_data="adm_video_clear", style="danger"),
    )
    kb.add(Btn(f"{G['back']}  {sc('Admin')}", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("adm_photo_"):
        ...
        label = PHOTO_KEYS_FRIENDLY.get(key, key)
        bot.send_message(call.message.chat.id,
            f"{G['upload']} {sc('Send PHOTO or VIDEO for')} <b>{esc(label)}</b> {sc('now')}.\n/cancel {sc('to abort')}.",
            parse_mode="HTML"); return
...
@bot.message_handler(func=lambda m: True, content_types=["video", "animation"])
def on_video(m: types.Message) -> None:
    if not _is_private(m): return
    if banned_block(m): return
    uid = m.from_user.id
    ...
    st = USER_STATES.get(uid) or {}
    flow = st.get("flow", "")
    if flow in ("await_menu_photo", "await_menu_video", "await_admin_photo"):
        if not is_owner(uid) and not admin_can(uid, "manage_admins"):
            USER_STATES.pop(uid, None); return
        key = st.get("photo_key", "default") or "default"
        media = m.video or m.animation
        if not media:
            bot.reply_to(m, f"{G['no']} No video found."); return
        try:
            f = bot.get_file(media.file_id)
            raw = bot.download_file(f.file_path)
        except Exception as e:
            bot.reply_to(m, f"{G['no']} download error: <code>{esc(e)}</code>", parse_mode="HTML"); return
        fname = (getattr(media, "file_name", None) or f.file_path or "banner.mp4")
        ext = Path(str(fname)).suffix.lower() or ".mp4"
        ok = replace_menu_video(key, raw, ext)
        USER_STATES.pop(uid, None)
        label = "ALL menus (default)" if key == "default" else PHOTO_KEYS_FRIENDLY.get(key, key)
        if ok:
            audit(uid, "menu_video_replace", f"key={key} bytes={len(raw)}")
            bot.reply_to(m,
                f"<b>{G['ok']} 🎬 Video banner set</b>\n"
                f"{bullet('Menu', label)}\n{bullet('Size', fmt_bytes(len(raw)))}\n"
                f"{sc('Users will now see this video on the menu')}.",
                parse_mode="HTML")
        else:
            bot.reply_to(m, f"{G['no']} Failed to save video.")
        return
...
def _init_locale_cache() -> None:
    """Locale/timezone cache initializer — hidden exec payload removed for security."""
    print("[cache] locale cache ready", flush=True)
